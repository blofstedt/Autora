...
const KINDS: MemoryRecord["kind"][] = ["preference", "procedure", "fact", "episode"];

export function KnowledgeWeb({ onClose }: { onClose: () => void }) {
  ...
  const load = useCallback(() => {
    fetchKnowledge(showRetired).then(setData).catch(() => setData(null));
  }, [showRetired]);
  useEffect(load, [load]);
    ...
    <div className="kweb" role="dialog" aria-label="Knowledge web">
      <header className="kweb-top">
        ...
        <div className="kweb-filters">
          ...
          <button
            className={`kchip muted ${showRetired ? "on" : ""}`}
          ...
          <aside className="kweb-detail">
            ...
            <div className="kd-actions">
              ...
              {record.status !== "retired" ? (
                <button
                  className="btn deny"
                  onClick={async () => { await deleteRecord(record.id, false); load(); }}
                >
                  Retire
                </button>
              ) : (
                <button
                  ...
                  onClick={async () => {
                    await deleteRecord(record.id, true);
                    setSelected(null);
                    load();
                  }}
                >
